...
const Setting: React.FC<{ isOpen: boolean, onToggle: (v: boolean) => void }> = ({ isOpen, onToggle}) => {
  ...
  return (
    <div>
      <div className={`h-[500px] border-solid shadow-lg overflow-y-auto rounded-xl transition-all fixed right-2 top-[80px] bg-white md:top-0 md:right-0 md:relative ${isOpen ? 'w-[300px] border' : 'w-0 border-none'}`}>
        <div className='p-4'>
